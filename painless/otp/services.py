...
class IranOTP:
    ...
    def send_token(self, to, token, template_id=settings.IRAN_OTP_TEMPLATE_ID):
        ...
        if successful:
            ...
            logger.critical(f'Security Token Failed for {to}.')
            return 'Security Token Failed.', False

    def send_message(self, to, message, template_id=settings.IRAN_OTP_TEMPLATE_ID):
        security_token, successful = self.__security_token()
        url = self.url + 'api/UltraFastSend'
...
iran_sms = IranOTP(settings.IRAN_OTP_USER_API_KEY, settings.IRAN_OTP_SECRET_KEY)
